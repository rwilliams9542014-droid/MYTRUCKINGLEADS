...
const PHONE_CANDIDATES = [
  ["phone", "business"], ["phoneNumber", "business"], ["telephone", "business"], ["tel", "business"],
  ["carrierPhone", "business"], ["businessPhone", "business"], ["primaryPhone", "primary"],
  ["contactPhone", "contact"], ["contact_phone", "contact"], ["phone1", "business"], ["phone2", "secondary"],
  ["alternatePhone", "secondary"], ["secondaryPhone", "secondary"], ["mainPhone", "business"],
  ["registrationPhone", "contact"], ["applicantPhone", "contact"], ["companyPhone", "business"],
  ["cellPhone", "mobile"], ["cell_phone", "mobile"], ["mobilePhone", "mobile"], ["mobile", "mobile"],
  ["cellphone", "mobile"], ["contactMobile", "mobile"],
  ["fax", "fax"], ["faxNumber", "fax"], ["carrierFax", "fax"], ["businessFax", "fax"], ["fax_phone", "fax"],
  ["carrier.phone", "business"], ["carrier.telephone", "business"], ["carrier.fax", "fax"],
  ["contact.phone", "contact"], ["contact.mobile", "mobile"], ["contactInfo.phone", "contact"],
  ["registration.phone", "contact"], ["business.phone", "business"], ["business.fax", "fax"],
  ["raw.phone", "business"], ["raw.telephone", "business"], ["raw.phoneNumber", "business"],
  ["raw.mobile", "mobile"], ["raw.mobilePhone", "mobile"], ["raw.cellPhone", "mobile"], ["raw.cell_phone", "mobile"],
  ["raw.fax", "fax"], ["raw.faxNumber", "fax"], ["raw.carrier.phone", "business"], ["raw.carrier.telephone", "business"],
  ["raw.carrier.fax", "fax"], ["raw.contact.phone", "contact"], ["raw.contact.mobile", "mobile"],
  ["raw.contactInfo.phone", "contact"], ["raw.registration.phone", "contact"], ["raw.business.phone", "business"],
  ["raw.business.fax", "fax"], ["raw.census.phone", "business"], ["raw.census.cell_phone", "mobile"], ["raw.census.fax", "fax"]
];
...
export function normalizePhoneDigits(value) {
  ...
  return digits.length === 10 && !/^(\d)\1{9}$/.test(digits) && !/^[01]/.test(digits) && !/^\d{3}[01]/.test(digits) ? digits : "";
}
...
function mergeEntry(existing, next) {
  ...
  return rankType(next.type) < rankType(existing.type) ? { ...existing, ...next } : existing;
}
...
export function dedupeContactNumbers(numbers = []) {
  ...
  numbers.filter(Boolean).forEach((entry) => {
    ...
    if (normalized) byDigits.set(normalized.digits, mergeEntry(byDigits.get(normalized.digits), normalized));
  });
...
export function collectContactNumbers(raw = {}, options = {}) {
  ...
  PHONE_CANDIDATES.forEach(([path, type]) => {
    const value = pathValue(source, path);
    if (Array.isArray(value)) value.forEach((item) => entries.push(normalizeEntry(item, type, { source: sourceName, sourceField: path })));
    else entries.push(normalizeEntry(value, type, { source: sourceName, sourceField: path }));
  });
...
export function collectContactNumbersFromAllSources(carrierSources = {}) {
  ...
  Object.entries(SOURCE_LABELS).forEach(([key, source]) => {
    ...
    if (!value) return;
    if (Array.isArray(value)) value.forEach((item) => entries.push(...collectContactNumbers(item, { source })));
    else entries.push(...collectContactNumbers(value, { source }));
  });
  return dedupeContactNumbers(entries);
}
